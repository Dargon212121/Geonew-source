...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags, int i)
{
    ...
    ImGui::RenderText(ImVec2(bb.Min.x + 10, bb.Min.y + 25 / 2 - label_size.y / 2), name);
...
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    return ImGui::BeginChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags, i);
}
