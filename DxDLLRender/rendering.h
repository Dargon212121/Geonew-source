...
private:
	...
	__declspec(noinline) bool InitRender(IDXGISwapChain* SwapChain)
	{
		static bool initialized; if (!initialized) {
			...
			TextEngine->CreateTextFormat(StrW(L"Visitor TT2 BRK"), NULL, DWRITE_FONT_WEIGHT_THIN, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_CONDENSED, 16.f, L"", &TextFormat);
...
public:
	...
	void Text(const Vector2 pos, const D2D1::ColorF clr, bool center, bool outline, const std::wstring_view text, Args&&... args) {
		...
		RET_CHK(TextEngine->CreateTextLayout(str.c_str(), str_len, TextFormat, Global::ScreenWidth, Global::ScreenHigh, &dwrite_layout));
